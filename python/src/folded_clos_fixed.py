...
from mininet.node import Host, Node, OVSKernelSwitch, RemoteController
...
def runPingTests(net, leaf, pod, fanout, dc, with_load, out_dir):
    ...
    for i in range(dc - 1):
        ...
        for i in range(len(destinations)):
            ...
            if with_load is True:
                createTraffic(shuffle, srchost)
                time.sleep(2)
                ping_out.write("\n--- Ping Test " + str(2 * i + 1) + " Results ---\n")

                #Populates the src host and destination host with arp entries 
                # of each other to avoid expensive calls to ARP
                dsthost = net.hosts[destinations[i]]
                srchost.setARP(dsthost.IP(), dsthost.MAC())
                dsthost.setARP(srchost.IP(), srchost.MAC())
                #ping_out.write("srchost: arp - IP: " + dsthost.IP() + " MAC: " + dsthost.MAC())
                #ping_out.write("dsthost: arp - IP: " + srchost.IP() + " MAC: " + srchost.MAC())

                ping_out.write(srchost.cmd("ping -c 5 " + dsthost.IP()))
                time.sleep(1)
                print("Ping Test " + str(2 * i + 2))
                ping_out.write("\n--- Ping Test " + str(2 * i + 2) + " Results ---\n")
                ping_out.write(srchost.cmd("ping -c 50 " + dsthost.IP()))
                time.sleep(4)
