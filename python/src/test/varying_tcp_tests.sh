...
datacenters=2
while [[ $datacenters -le $maxdatacenters ]]
do
trials=1
...
do
    ...
    cd ..
    sudo python folded_clos.py --dc $datacenters --leaf 2 2 2 --spine 2 2 2 --pod 3 3 2 --ratio 2 2 1 --fanout 3 3 3 --test true --run_ping false --with_traffic $traffic --high_latency $highlatencylink
    cd test
    ...
    datacentersubdir="/${datacenters}_datacenters"
    pingdir=$realdir$datacentersubdir$realsubdir
    if [[ $error != "" ]]
    then
    ...
    then
        ...
        mv -vf tcp_receive_no_load.out $pingdir
	mv -vf tcp_receive_no_load2.out $pingdir
	mv -vf tcp_receive_no_load3.out $pingdir
	mv -vf tcp_receive_no_load4.out $pingdir
	mv -vf tcp_receive_no_load5.out $pingdir
	mv -vf tcp_receive_no_load6.out $pingdir
	mv -vf tcp_receive_no_load7.out $pingdir
	mv -vf tcp_receive_no_load8.out $pingdir
	mv -vf tcp_receive_no_load9.out $pingdir
	mv -vf tcp_receive_no_load10.out $pingdir
	...
	mv -vf tcp_receive_no_load.out $dups
   	mv -vf tcp_receive_no_load2.out $dups 
   else
        acceptable=0
